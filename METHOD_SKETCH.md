...
method space comprehensively rather than to advocate for one approach.
...
| sPLS-DA | PLS-DA with L1 sparsity on loadings — explicitly selects a subset of wavenumbers per component | Latent scores are continuous; final prediction is discrete | Yes, and sparser than PLS-DA — selected wavenumbers only | Not in scikit-learn; `mixOmics` (R) or custom Python |
...
correlation, robust to outlier lines). All computed at the line level.

**Positive control:** starvation resistance EMMeans — signal confirmed in the pre-print
