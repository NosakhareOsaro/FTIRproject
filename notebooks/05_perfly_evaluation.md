...
1. **Label leakage:** all flies from the same line share the same EMMean label. A fly in the test fold has the same label as its siblings in the training fold — the model can partially infer the test label by recognising the line.
...
- The `y_train` target for each fly is its line's EMMean, repeated ~16 times. This is the only possible target given the destructive assay design — individual fly phenotypes are not available
